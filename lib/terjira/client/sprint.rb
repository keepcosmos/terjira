require_relative 'base'

module Terjira
  module Client
    class Sprint < Base
      class << self
        delegate :build, to: :resource

        def all(board_id, options = {})
          url = "/rest/agile/latest/board/#{board_id}/sprint"
          params = options.slice(:state, :maxResults)
          url += "?#{URI.encode_www_form(params)}" if params.present?
          resp = client.get(url).body
          result = JSON.parse(resp)
          result["values"].map { |value| build(value) }
        end

        def find(sprint_id)
          resp = client.get("/rest/agile/latest/sprint/#{sprint_id}").body
          build JSON.parse(resp)
        end

        def find_active(board_id)
          resp = client.get("/rest/agile/latest/board/#{board_id}/sprint?state=active").body
          result = JSON.parse(resp)
          result["values"].map { |value| build(value) }.first
        end
      end
    end
  end
end
